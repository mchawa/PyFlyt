...
class Logger:
    ...
    def add(self, timestamp, state, action, reward):
        ...
        """
        target_pos = state[0:3] + state[12:15]
        target_psi = state[8] + state[15]
    ...
    def log_episode(self):
        ...
        avg_reward = np.sum(np.array([entry[33] for entry in self.buffer]))
        ...
        fig2.suptitle(
            "Scenario Reward: {:.2f}".format(avg_reward),
            fontsize=12,
            fontweight="bold",
        )
